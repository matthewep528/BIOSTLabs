...
hflash <- read_xlsx(here("Lab5", "hflash.xlsx")) %>%
    mutate(
        AgeGroup = factor(ageg - 1), # making dummy variable for age group
    ) %>%
    drop_na()

kable(summary(hflash))

# note: drop_na(df) doesn't save the data frame
# if you want to drop the missing values and save the data so that no values are missing you have to save it with a name (either the same name as the data frame to update the data or a new name to indicate it is the complete cases only)

# example:
# df.complete = drop_na(df)
