...
```{r, message=FALSE}
# load libraries (you may need to install these if you never have before)
library(tidyverse)
library(readxl)
library(car)
```
...
# (don't forget to save it as an object with a name)
...
We will start by fitting separate models to see if any of the three predictors of interest are associated with the outcome.

### Part 1) Fitting SLR models

__Fit separate simple linear regression models for assessing the association of total cholesterol level (cholesterol), total triglyceride level (triglyceride) and sinking pre-beta lipoprotein (spbl) with HDL (HDL) (mg/dL). Write the estimated regression equation and interpret the slope coefficient for each of the three models.__

```{r}
# REMINDER: syntax to fit SLR model is lm(outcome ~ predictor, data = data_name)
